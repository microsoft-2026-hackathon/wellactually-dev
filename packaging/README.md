...
현재 세션의 정확한 Workspace 아래 `.wellactually/knowledge/`에 새 Markdown 파일을
만들고 링크를 반환합니다. MCP host가 양식 확인을 지원하면 저장 위치를 확인한 뒤
...
`YYYY-MM-DD-ascii-slug-uuid.md` 파일을 만들며, 연결된 대상 디렉터리를 거부하고
기존 파일을 덮어쓰지 않습니다. 일반 쓰기 권한으로 우회하지 않습니다.

...
Wellactually의 첫 제품 형태는 별도 Chat UI가 아니라 VS Code의 Custom Agent와
Agent Host를 사용한다. 제품은 Pair와 Driver의 대화를 분리하면서도, Pair가
Driver의 작업 과정과 사용자가 전달한 구현 지시를 읽고 다음 논의를 이어 갈 수
있게 한다.

## Agent Host 구성

하나의 Agent Host 세션 안에 Pair Chat과 Driver Chat을 둔다.

```text
Agent Host 세션
├─ Pair Chat
│  └─ 사용자가 선택한 Wellactually 페어링 모드
└─ Driver Chat
   ├─ Pair가 최초 한 번 생성
...
두 Chat은 서로의 대화 기록을 자동으로 상속하지 않는다. 대신 같은 Agent Host
세션에 속하므로 Workspace와 worktree를 공유한다. Driver가 변경한 코드는 Pair가
Workspace에서 확인할 수 있고, Pair는 Agent Host의 세션 관리 도구를 사용해
Driver Chat의 최근 대화 맥락도 읽을 수 있다.

데이터 접근은 비대칭으로 구성한다.

```text
Pair ──읽기──> Driver Chat
Driver ──접근 불가──> Pair Chat
```

Pair는 Driver Chat에 메시지를 보내거나 작업을 위임하지 않는다. Driver에게 어떤
작업을 맡길지 결정하고 실제 구현 지시를 작성하는 주체는 사용자다.

## Driver Chat 생성과 사용 흐름

1. 사용자가 Pair mode를 선택하고 Pair Chat에서 문제, 방향과 Scope을 논의한다.
2. Pair는 Agent Host의 `create_session` 도구를 `currentSession` 관계로 호출해 같은
   Agent Host 세션 안에 Driver용 Chat을 만든다.
3. `create_session`에는 초기 프롬프트가 필수이므로, 실제 작업을 지시하지 않는
   짧은 안내를 넣는다.

   ```text
   Wellactually Driver 작업을 위한 Chat입니다.
   사용자가 직접 구현 지시를 전달할 때까지 기다리세요.
   ```

4. 생성 결과에 포함된 `Open Session` 링크를 통해 사용자가 Driver Chat을 연다.
5. `create_session`은 대상 Custom Agent를 지정할 수 없으므로 사용자가
   **Wellactually Driver**를 직접 선택한다.
6. 사용자는 Pair와 논의한 판단을 자신의 말로 구현 지시에 담아 Driver에게
   전달한다.
7. Driver는 국소적인 설계, 구현과 검증을 수행한다.
8. 사용자는 기존 Pair Chat으로 돌아가 논의를 계속한다.
9. Pair는 생성 시 받은 Driver Chat 링크로 `get_session_context`를 호출해 사용자
   지시, Driver 응답과 도구 사용 맥락을 읽고, Workspace의 실제 변경도 확인한다.

이 흐름은 필요한 만큼 반복할 수 있다. Pair가 Driver Chat을 다시 만들거나 기존
Driver Chat에 후속 메시지를 보내는 방식으로 반복하지 않는다. 사용자가 같은
Driver Chat에서 다음 구현 지시를 내리고 Pair는 그 결과를 읽는다.
...
- `create_session`: 같은 Agent Host 세션 안에 Driver Chat을 최초 한 번 만든다.
- `list_sessions`: 관련 Agent Host 세션의 상태와 메타데이터를 확인한다.
- `get_session_context`: 생성한 Driver Chat의 최근 대화를 읽는다.
...
Pair에는 코드 편집, 명령 실행, 다른 Agent 호출, 다른 Chat으로 메시지를 보내는
도구를 제공하지 않는다.

### Driver 도구

Driver에는 Workspace 읽기와 검색, 코드 편집, 명령 실행 및 검증 도구를 제공한다.
다른 Agent Host 세션이나 Chat을 조회하고 읽고 제어하는 도구는 제공하지 않는다.
...
MVP에는 MCP가 필요하지 않다. Driver Chat 접근에는 Agent Host 내장 세션 관리
도구를 사용한다.
...
직접 작성하는 과정이 제품 경험의 일부이기 때문이다. Driver Chat 생성 시
...
- `create_session`은 `prompt`와 `title`을 필수로 받고 생성 직후 첫 요청을
  실행한다. 빈 Driver Chat만 만드는 동작은 현재 지원되지 않는다.
...
- Pair가 특정 Driver Chat을 다시 읽으려면 생성 결과의 Chat별 `openLink`를
  유지해야 한다. Context compaction이나 세션 복원 뒤에도 이 연결을 안정적으로
  재사용할 수 있는지 실제 환경에서 검증해야 한다.
...
- 같은 Agent Host 세션 안에서 분리된 Pair Chat과 Driver Chat
